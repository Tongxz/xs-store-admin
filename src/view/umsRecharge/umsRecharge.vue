<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="充值金额" prop="amount" width="120" >
          <template #default="scope" >
            <p v-if="scope.row.sign">{{"+"+scope.row.amount}}</p>
          </template>
        </el-table-column>
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="text" icon="edit" size="small" class="table-button" @click="updateRechargeFunc(scope.row)">变更</el-button>
...
watch(() => route.params.id, (id) => {
  console.log(id)
  searchInfo.value.memberId = Number(id)
  getTableData()
})
// 自动化生成的字典（可能为空）以及字段
const formData = ref({
...
const tableData = ref([])
const searchInfo = ref({ memberId: Number(route.params.id) })

// 重置
const onReset = () => {
  searchInfo.value = {memberId: Number(route.params.id) }
}
...
const updateRechargeFunc = async(row) => {
    ...
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rerecharge
...
const enterDialog = async () => {
  formData.value.memberId = Number(route.params.id)
      let res
      switch (type.value) {
        case 'create':
